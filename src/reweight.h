...
void reweight(
        ...
        data_iterator K,    //->cK independent real column vectors on the same rP points - given column by column - (and tested only in the case with sum over each row is 1)
        integer rP,
        integer ldk,// the stride in K
        integer cK,
        doublereal probability_zero_tolerance// UNUSED an empirical measure of the errors rounding gives to probability measure calculations
    );
